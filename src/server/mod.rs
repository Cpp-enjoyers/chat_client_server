...
use chat_common::messages::{
    ...
    ErrorMessage, JoinChannel, MessageData, SendMessage,
};
        ...
        if let Some(kind) = message.message_kind {
            match kind {
                ...
                MessageKind::CliCancelReg(..) => {
                    self.msg_clicancelreq(&mut replies, &cli_node_id);
                }
                ...
                MessageKind::CliJoin(data) => {
                    self.msg_clijoin(&mut replies, data, &cli_node_id);
                }
                MessageKind::CliLeave(..) => {
                    self.msg_clileave(&mut replies, &cli_node_id);
                }
                MessageKind::SendMsg(msg) => {
                    self.msg_sendmsg(&mut replies, &cli_node_id, msg);
                }
                MessageKind::Err(e) => {
...
impl ChatServerInternal {
    ...
    fn msg_clijoin(
        ...
        if channelinfo.1.contains(&cli_node_id) {
            ...
            for val in self
                .channel_info
                .iter_mut()
                .filter(|(id, x)| **id != 0x1 && **id != u64::from(*cli_node_id) << 32 | 0x8 && **id != channel_id)
            {
    ...
    fn msg_clicancelreq(&mut self, replies: &mut Vec<(NodeId, ChatMessage)>, cli_node_id: &NodeId) {
    ...
    fn msg_clileave(&mut self, replies: &mut Vec<(NodeId, ChatMessage)>, cli_node_id: &NodeId) {
        info!(target: format!("Server {}", self.own_id).as_str(), "Received leave request from client {cli_node_id}");
        for val in self
            .channel_info
            .iter_mut()
            .filter(|(id, x)| **id != 0x1 && **id != u64::from(*cli_node_id) << 32 | 0x8)
        {
            trace!(target: format!("Server {}", self.own_id).as_str(), "Removing client {cli_node_id} from channel {}", val.0);
            val.1 .1.remove(&cli_node_id);
        }
